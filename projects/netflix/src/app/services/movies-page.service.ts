...
import { Media, MovieGenre, MovieGenreName, Results } from '../types/types';
...
export class MoviesPageService {
  ...
  public getMovies(genre: MovieGenreName): Observable<Media> {
    return this.http
      .get<Results<Media>>(this.connection.MOVIES_GENRES_URL + this.findGenreId(genre))
      .pipe(
        map(({ results }) => results)
       );
  }
